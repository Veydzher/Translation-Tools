...
def export_strings(file_path:str, strings:dict, detailed_output:bool = True, export_none_strings:bool = False):
    try:
        exported_strings_count = 0
        exported_none_strings_count = 0
        exported_strings_fail = []
            ...
            for name in sorted(strings):
                if strings[name] is not None:
                    ...
                    if export_none_strings:
                        ...
                        exported_strings_fail.append(name)
        
        total_exported_strings = exported_strings_count + exported_none_strings_count
    ...
    if detailed_output:
        if exported_strings_fail:
            with open("failed_export_strings.txt", "w+", encoding="utf-8") as ef:
                ef.write("Failed to export the following strings:\n")
                for esf in exported_strings_fail:
                    ef.write("\t{}\n".format(esf))
        
        print_box([
            f"Total strings: {len(strings)}",
            f"Total exported strings: {total_exported_strings}",
            f"Exported whole strings: {exported_strings_count}",
            f"Exported empty strings: {exported_none_strings_count}",
            f"Strings have been saved to {file_path} file!"
        ])
    else:
        print_box([
        f"Total strings: {len(strings)}",
        f"Exported strings: {total_exported_strings}",
        f"Strings have been saved to {file_path} file!"
        ])
    return True
...
def import_strings(source_file: str, output_file: str, strings: dict, detailed_output:bool = False):
    try:
...
def main():
    ...
    try:
        ...
        if choice == 1:
            ...
            if strings:
                export_strings(output_file, strings, True)
            else:
            ...
            if modified_strings:
                import_strings(xml_file_path, output_file, modified_strings, True)
            else:
