...
def print_box(lines:list[str], h_padd:int = 4, v_padd:int = 2, alignment:str = "center"):
    """_summary_
...
def import_strings(source_file: str, output_file: str, strings: dict):
    try:
        ...
        if import_strings_count != 0:
            print(f"Imported strings: {import_strings_count}")
            
            tree.write(f"{output_file}.xml", encoding="utf-8", xml_declaration=True)
            print(f"{output_file}.xml created successfully!")
        else:
...
def main():
    print_box(["XML to STRINGS Converter Program by veydzh3r", "Github: https://github.com/Veydzher/Translation-Tools"], 6, 2)
    try:
